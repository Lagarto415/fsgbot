...
function parseEventTime(date, time, timezone = 'Europe/Berlin') {
    // Combines date and time and interprets it in the given timezone
    return moment.tz(`${date} ${time}`, 'DD/MM/YYYY HH:mm', timezone).toDate();
}
...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (interaction.customId === 'event-type') {
            ...
            const options = trackOptions.map(option => new StringSelectMenuOptionBuilder()
            ...
            eventDateTime = parseEventTime(date, time);

            const eventEndTime = moment(eventDateTime).add(2, 'hours').toDate();
            const eventEndTimestamp = eventEndTime.getTime() / 1000;
            ...
            let embed = new EmbedBuilder()
                ...
                .setTimestamp()
                .setImage(trackImg)
                .addFields({ name: 'Zeit', value: `<t:${eventTimestamp.toString()}> - <t:${eventEndTimestamp.toString()}> \n :watch: <t:${eventTimestamp.toString()}:R>` })
                .addFields(
            ...
            const row = new ActionRowBuilder()
                .setComponents(
                    new ButtonBuilder().setStyle('Success').setCustomId('event-accept').setEmoji('✔️'),
                    new ButtonBuilder().setStyle('Danger').setCustomId('event-deny').setEmoji('✖️'),
                    new ButtonBuilder().setStyle('Secondary').setCustomId('event-tentitive').setEmoji('❓')
                );
