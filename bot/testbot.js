...
client.once('ready', () => {
    ...
    const guild = client.guilds.cache.get('1121488994149081110'); // Assuming the bot is only in one guild

    if (guild) {
      welcomeChannel = guild.channels.cache.find(channel => channel.id === '1224433505186873495');
      rulesChannel = guild.channels.cache.find(channel => channel.id === '1141491969709842593');
      driverApplicationsChannel = guild.channels.cache.find(channel => channel.id === '1224436130082979890');
      goodbyeChannel = guild.channels.cache.find(channel => channel.id === '1224749053204631593');
      modlog = guild.channels.cache.find(channel => channel.id === '1224754111031935006');
      numberchannel = guild.channels.cache.find(channel => channel.id === '1225127200689229835');
      ttvTextChannel = guild.channels.cache.find(channel => channel.id === '1225841643123507333');
  } else {
